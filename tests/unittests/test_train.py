...
def test_my_train():
    def setUp(self):
        ...
        self.mock_config = {
            "data": {"processed_path": "./mock_dataset"},
            "train": {
                "batch_size": 4,
                "num_workers": 4,
                "lr": 0.001,
            ...
            "model": {"num_classes": 1000, "save_path": "./mock_model.pth"},
        }
    ...
    def test_training_process(self):
        ...
        try:
            ...
            model = ResNetModel(
                num_classes=self.mock_config["model"]["num_classes"], lr=self.mock_config["train"]["lr"]
            )
            # dataloader = DataLoader(self.mock_config['dataset_path'], self.mock_config['batch_size'])
            dataloader = DataLoader(
                self.mock_config["processed_path"],
                batch_size=self.mock_config["train"]["batch_size"],
                shuffle=False,
                num_workers=self.mock_config["train"]["numworkers"],
            )
